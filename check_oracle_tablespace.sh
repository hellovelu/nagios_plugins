...
ORACLE_USER=""
ORACLE_PASS=""
ORACLE_CREDENTIALS="/etc/infomentum/config/nagios/nrpe/oracle_db"


if [ -z "$ORACLE_PASS" ] ; then
        # Adjust to taste
        for nagios_db in  $ORACLE_CREDENTIALS
        do
        [ ! -f $nagios_db ] && continue
                while read USER PWD junk;
                do
                        if [[ ! "$USER" =~ (^#|^$) ]] ; then
                                IFS=':' read  USER PWD  <<< "$USER";
                                ORACLE_USER=$USER;
                                ORACLE_PASS=$PWD;
                        fi
                done < $nagios_db
